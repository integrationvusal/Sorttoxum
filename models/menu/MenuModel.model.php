<?php
	class MenuModel extends CRUDModel {
		...
		public function __construct() {
			...
			$this->clickable = new ModelSelectField("clickable", "Clickable", Array(
				1 => "Clickable",
				0 => "Not clickable"
			), true, true);
			$this->clickable->common = true;

			$this->newColumn = new ModelSelectField("newColumn", "Start on new col", Array(
				0 => "No",
				1 => "Yes"
			), true, true);
			$this->newColumn->common = true;

			$this->class = new ModelSelectField("class", "Style class", Array(
				3 => "Right aligned",
				2 => "Full width",
				1 => "No submenu",
				0 => "Default"
			), true, true);
			$this->class->common = true;
		}
		...
		public static function getChildsForBoth($pageId, $lang = false) {
			$currentPage = self::get($pageId, $lang);
			$menuRelationsTable = MenuRelationsModel::getTableName();
			$menuModelTable = MenuModel::getTableName();
			$sql = "SELECT `".$menuRelationsTable."`.`order`, 
		...
		public static function getParentFor($pageId, $lang = false){
			$currentPage = self::get($pageId, $lang);
			$menuRelationsTable = MenuRelationsModel::getTableName();
			$menuModelTable = MenuModel::getTableName();
			$sql = "SELECT `".$menuRelationsTable."`.`parentId` FROM `".$menuRelationsTable."`
			...
			if (count($parentId)) return $parentId[0]['parentId'];
			return 0;
		}


		public static function getParentForNew($pageId, $lang = false){
			$currentPage = self::get($pageId, $lang);
			$menuRelationsTable = MenuRelationsModel::getTableName();
			$menuModelTable = MenuModel::getTableName();
			$sql = "SELECT `".$menuRelationsTable."`.`parentId` FROM `".$menuRelationsTable."`
					INNER JOIN `".$menuModelTable."`
					ON `".$menuRelationsTable."`.`id` = `".$menuModelTable."`.`treeItemId`
					WHERE `".$menuModelTable."`.`treeItemId` = '{#1}' AND `".$menuModelTable."`.`lang_id` = '{#2}'";
			$parentId = self::fQuery($sql, Array($currentPage->treeItemId->value, $lang));
		...
		public static function getParentByTreeItemId($treeItemId, $lang = false){
			$menuRelationsTable = MenuRelationsModel::getTableName();
			$sql = "SELECT `parentId` FROM `" . $menuRelationsTable . "` WHERE `id` = '{#1}'";

			$parentId = self::fQuery($sql, Array($treeItemId));
			...
			foreach ($pages as $k => $mItem) {
				...
				switch ($mItem->type->value) {
					...
					case 2:
						...
						break;
					case 3:
						$pages[$k]->url->value = Application::$settings['url'] . "/" . Application::$storage['lang'] . "/" . $mItem->link->value;
						break;
					case 4:
